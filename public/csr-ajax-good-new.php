...
class SR_AJAX_New_Good extends AJAX_Handler {
    function callback() {
      $tab_reports = (new School_Report_Db_Table)->get_table("good_students");
      $res = $tab_reports->insert($_POST);
      $insert_id = $tab_reports->get_insert_id();
      wp_send_json_success(json_encode(array("result" => $res, "id_good" => $insert_id)));
    }
}

new SR_AJAX_New_Good('new_good_student');
